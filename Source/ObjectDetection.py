...
# ====== CONFIG ======
DEFAULT_MODEL_PATH = r"D:\TOYPROJECT\OS\Model\OBD\OBD.engine"  # or .pt
CONF_DEFAULT = 0.25
IOU_DEFAULT = 0.45
LINE_THICKNESS = 1
FONT_SCALE = 0.5
PADDING = 6
WINDOW_TITLE = "YOLOv11 — OBB only"
# =====================
...
class OBBOnlyApp:
    def __init__(self, master):
        ...
        self.engine_imgsz = None  # 엔진 입력 크기는 로드 후 자동 감지/보정
        ...
        self.btn_infer = tk.Button(
            actions, text="Run Inference (OBB)", width=18, command=self.run_inference, state=tk.DISABLED
        )
    ...
    def save_result(self):
        if self.vis_bgr is None:
            ...
            return
        initial = os.path.splitext(os.path.basename(self.image_path or "result"))[0] + "_obb.png"
    ...
    def load_model(self):
        ...
        # 엔진일 때는 task를 OBB로 강제 지정
        if self.is_engine:
            try:
                self.model = YOLO(self.model_path, task="obb")
            except TypeError:
                self.model = YOLO(self.model_path)
            try:
                setattr(self.model, "task", "obb")
                if hasattr(self.model, "overrides") and isinstance(self.model.overrides, dict):
                    self.model.overrides["task"] = "obb"
                if hasattr(self.model, "args") and isinstance(self.model.args, dict):
                    self.model.args["task"] = "obb"
            except Exception:
                pass
        else:
            self.model = YOLO(self.model_path)
        ...
        if self.is_engine:
            ...
            if not self.engine_imgsz:
                self.engine_imgsz = 320  # 기본 후보, 런타임에서 자동 보정됨

        self.model_label.config(
            text=f"Model: {self.model_path}  (Device: {self.device})  [Mode: OBB]"
        )
    ...
    def run_inference(self):
        ...
        def _infer_thread():
            try:
                ...
                try:
                    if hasattr(self.model, "warmup"):
                        self.model.warmup(
                            ...
                            device=(
                                0
                                if (self.is_engine and torch.cuda.is_available())
                ...
                # ★ 엔진인데 obb가 비어 있으면 DET로 export된 엔진임
                if self.is_engine and getattr(res, "obb", None) is None:
                    messagebox.showerror(
                        "Engine is not OBB",
                        "This TensorRT engine did not return OBB outputs.\n\n"
                        "It was likely exported as DET.\n"
                        "Please re-export from an OBB .pt model.\n\n"
                        "Tip) Use: YOLO('your_obb.pt', task='obb').export(format='engine', imgsz=640, half=True, device=0)"
                    )
                    return
                ...
                # 그릴 때는 항상 원본(res.orig_img) 기준 → 완전 정렬
                base = getattr(res, "orig_img", None)
                if base is None:
                    base = self.src_bgr

                vis = draw_obb_on_image(base, res, safe_names)
                ...
                print(
                    f"[INFO] OBB-only imgsz={imgsz} device={'cuda' if torch.cuda.is_available() else 'cpu'} "
